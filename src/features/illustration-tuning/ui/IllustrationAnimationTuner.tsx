...
export const IllustrationAnimationTuner = ({
  ...
  return (
    <aside
      ...
      className={`fixed inset-y-0 right-0 z-50 h-screen w-[min(24rem,100vw)] overflow-y-auto border-l border-border-strong bg-panel p-4 text-cream-white shadow-[0_24px_80px_var(--color-footer-shadow)] transition-transform duration-75 ease-out ${isPlaying ? 'translate-x-full' : 'translate-x-0'}`}
    >
      <button
        type="button"
        className="ml-auto block px-2 font-mono text-sm text-text-muted"
      ...
      <div className="mt-2 space-y-3">
        ...
        <fieldset disabled={tunerState.selectedIsLocked} className="disabled:opacity-40">
          <WordAnimationControls
            animation={panel.selectedAnimation}
            illustrationKind={tunerState.selectedIllustrationKind}
            onChange={tunerState.setAnimation}
            text={typeof panel.selectedSection.illustrateWith === 'string' ? panel.selectedSection.illustrateWith : ''}
          />
        </fieldset>
